...
export async function initMap(hoteles: any[], museos: any[]) {
  ...
  // FUNCIÓN CLAVE: Extrae el nombre del departamento de forma segura
  const getDeptoName = (r: any) => {
    if (!r.entidades) return null;
    // Maneja si entidades es un objeto o un array de un objeto
    const ent = Array.isArray(r.entidades) ? r.entidades[0] : r.entidades;
    return ent?.departamento || null;
  };
  ...
  const getWeight = (name: string) => {
    ...
    const h = hoteles
      .filter((r) => {
        const d = getDeptoName(r);
        return d && normalizar(d) === norm;
      })
  ...
  const pesos = sanjuan.features.map((f: any) => getWeight(f.properties.departamento));
  const maxVal = Math.max(...pesos, 1); // Bajamos a 1 para que el color se note más si hay pocos datos
  ...
  g.selectAll("path")
    ...
    .on("mousemove", (event: any) => {
      tooltip.style("top", (event.layerY - 35) + "px").style("left", (event.layerX + 10) + "px");
    })
...
function actualizarInfoDepto(nombre: string | null, hoteles: any[], museos: any[]) {
  ...
  const eEl = document.getElementById("depto-entidades");
  ...
  if (!nombre) {
    ...
    e = new Set([...hoteles.map(r => r.entidad_id), ...museos.map(r => r.entidad_id)]).size;
  } else {
    if (nameEl) nameEl.textContent = nombre;
    const norm = normalizar(nombre);
    const filtradosH = hoteles.filter((r) => {
        const d = getDeptoName(r);
        return d && normalizar(d) === norm;
    });
    const filtradosM = museos.filter((r) => {
        const d = getDeptoName(r);
        return d && normalizar(d) === norm;
    });
    ...
    e = new Set([...filtradosH.map(r => r.entidad_id), ...filtradosM.map(r => r.entidad_id)]).size;
  }
